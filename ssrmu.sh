...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/fei5seven/doubi/master/bbr.sh; then
...
Set_config_user(){
	...
	read -e -p "(默认: i):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="i"
...
Set_config_port(){
	...
	read -e -p "(默认: 12450):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="12450"
...
Set_config_password(){
	echo "请输入要设置的用户 密码"
	read -e -p "(默认: fei5seven):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="fei5seven"
...
Download_SSR(){
	cd "/usr/local"
	wget -N --no-check-certificate "https://github.com/fei5seven/shadowsocksr/archive/manyuser.zip"
	#git config --global http.sslVerify false
	#env GIT_SSL_NO_VERIFY=true git clone -b manyuser https://github.com/fei5seven/shadowsocksr.git
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/fei5seven/doubi/master/service/ssrmu_centos -O /etc/init.d/ssrmu; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/fei5seven/doubi/master/service/ssrmu_debian -O /etc/init.d/ssrmu; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssrmu
...
# 封禁 BT PT SPAM
BanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/fei5seven/doubi/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh banall
	rm -rf ban_iptables.sh
}
# 解封 BT PT SPAM
UnBanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/fei5seven/doubi/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh unbanall
	rm -rf ban_iptables.sh
}
Set_config_connect_verbose_info(){
	SSR_installation_status
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/fei5seven/doubi/master/ssrmu.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	cd "${file}"
	wget -N --no-check-certificate "https://raw.githubusercontent.com/fei5seven/doubi/master/ssrmu.sh" && chmod +x ssrmu.sh
